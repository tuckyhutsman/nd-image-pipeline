...
function PipelineEditor({ onPipelineSaved, editPipelineId, onBack }) {
  ...
  const fetchSingleAssetPipelines = async () => {
    try {
      const response = await apiClient.get('/pipelines');
      // Filter for single-asset pipelines only
      const allPipelines = response.data;
      const singleAsset = allPipelines.filter(p => {
        const config = typeof p.config === 'string' ? JSON.parse(p.config) : p.config;
        return config.type === PIPELINE_TYPES.SINGLE_ASSET;
      });
      setSingleAssetPipelines(singleAsset);
    } catch (err) {
      console.error('Failed to load single-asset pipelines:', err);
    }
  };
  ...
  const handleSaveMultiAsset = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        await apiClient.put(`/pipelines/${editingId}`, payload);
        setSuccess('Multi-asset pipeline updated');
      } else {
        await apiClient.post('/pipelines', payload);
        setSuccess('Multi-asset pipeline created');
      }
      ...
      setError('Error saving multi-asset pipeline: ' + err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const addComponent = (singleAssetId) => {
    ...
    if (multiAssetForm.components.find(c => c.ref === singleAssetId)) {
      setError('This pipeline is already added');
  ...
  const removeComponent = (ref) => {
    const updatedComponents = multiAssetForm.components
      .filter(c => c.ref !== ref)
      .map((c, i) => ({ ...c, order: i + 1 }));
      ...
      <div className="pipeline-editor">
        ...
        <form className="pipeline-form">
          ...
          <div className="form-section">
            ...
            <p className="info-text">Multi-asset pipelines compose multiple single-asset pipelines. Each input file generates one output per component.</p>
            
            <div className="components-selector">
              <label>Add Pipeline Component</label>
              <select 
                onChange={(e) => {
            ...
            {multiAssetForm.components.length > 0 && (
                ...
                {multiAssetForm.components.map((comp, idx) => {
                  ...
                  return (
                    <div className="component-item" key={comp.ref}>
                      <div className="component-order">{idx + 1}</div>
                      <div className="component-info">
                        <span className="component-name">{pipeline?.name || 'Unknown Pipeline'}</span>
                        {config?.suffix && <span className="component-suffix">→ {config.suffix}</span>}
                      </div>
                      <div className="component-actions">
                        ...
                        <button 
                          ...
                          disabled={idx === multiAssetForm.components.length - 1}
                          className="btn-icon"
                          title="Move down"
                        >
                          ↓
                        ...
                        <button 
                          type="button"
                          onClick={() => removeComponent(comp.ref)}
            ...
            {multiAssetForm.components.length === 0 && (
              <div className="empty-state">
                <p>No components added yet. Select pipelines above to build your multi-asset workflow.</p>
              </div>
            )}
          </div>
          ...
          <div className="form-actions">
            <button 
              ...
              onClick={handleSaveMultiAsset}
              disabled={loading || multiAssetForm.components.length === 0}
            >
              {loading ? 'Saving...' : editingId ? 'Update Multi-Asset Pipeline' : 'Create Multi-Asset Pipeline'}
            </button>
            <button type="button" className="btn btn-secondary" onClick={() => { setMode('list'); setEditingId(null); }}>
              Cancel
            </button>
